...
// A cadeia de resolução (NEXT_PUBLIC_SITE_URL → X-Forwarded-Host → Host →
// fallback), incluindo a allow-list ALLOWED_INVITE_HOSTS, foi EXTRAÍDA daqui
// para `@/lib/base-url` — `/auth/callback` precisava exatamente da mesma coisa,
// e duas cópias de uma decisão de HOST divergiriam. O comportamento é idêntico
// ao anterior, incluindo o fallback para o domínio de marketing quando a
// requisição não traz Host algum. Agora tem teste.
